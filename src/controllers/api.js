...
const Linkedin = require('node-linkedin')(process.env.LINKEDIN_ID,
	process.env.LINKEDIN_SECRET, process.env.LINKEDIN_CALLBACK_URL);
...
exports.getFoursquare = async (req, res, next) => {
	...
	axios.all([
		...
		then(axios.spread(
			(trendingVenuesRes, venueDetailRes, userCheckinsRes) => {
...
exports.getTumblr = (req, res, next) => {
	...
	client.blogPosts('mmosdotcom.tumblr.com', { type: 'photo' },
		(err, data) => {
...
exports.getScraping = (req, res, next) => {
	request.get('https://article.ycombinator.com/', (err, request, body) => {
		if (err) { return next(err); }
...
exports.getGithub = async (req, res, next) => {
	...
	try {
		const { data: repo } = await github.repos.get(
			{ owner: 'sahat', repo: 'hackathon-starter' });
...
exports.getNewYorkTimes = (req, res, next) => {
	...
	request.get({ url: 'http://api.nytimes.com/svc/books/v2/lists', qs: query },
		(err, request, body) => {
			if (err) { return next(err); }
	...
	try {
		...
		const artist = {
			...
			image: artistInfo.image
				? artistInfo.image.slice(-1)[0]['#text']
				: null,
