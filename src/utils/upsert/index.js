...
async function upsertManyPosts(publicationID, newPosts, schemaField) {
	...
	const schema = schemaField === 'rss' ? Article : null;
	...
	for (const post of newPosts) {
		...
		if (post.fingerprint in existingPostsMap) {
			...
			operations.push(
				{ updateOne: { filter: filter, update: dataWithoutId } });
	...
	for (const [k, v] of Object.entries(operationMap)) {
		statsD.increment(`khabarkhun.handle_rss.${schemaField}.${k}`, v.length);
	}
...
function normalizedDiff(existingPost, newPost) {
...
function postChanged(existingPost, newPost) {
	return normalizedDiff(existingPost, newPost).length !== 0;
}

module.exports.normalizePost = normalizePost;
module.exports.normalizedDiff = normalizedDiff;
module.exports.postChanged = postChanged;
module.exports.upsertManyPosts = upsertManyPosts;